...
public static class TrafficMonitor
{
    private class PacketStats
    {
        ...
        public void Record(int bytes)
        {
            ...
            do
            {
                ...
                bytes < oldMin && Interlocked.CompareExchange(ref MinBytes, bytes, oldMin) != oldMin
            );
            int oldMax;
            do
            {
                ...
                bytes > oldMax && Interlocked.CompareExchange(ref MaxBytes, bytes, oldMax) != oldMax
            );
        }
    }
    ...
    {
        lock (History)
            return new List<ThroughputEntry>(History);
    }
        ...
        return new
        {
            incoming = IncomingStats
                .Select(kvp => new
                {
                    ...
                    avg = Interlocked.Read(ref kvp.Value.Count) > 0
    ...
    private static void AppendStats(
        StringBuilder sb,
        ConcurrentDictionary<string, PacketStats> statsDict
    )
    {
        var sorted = statsDict
            .OrderByDescending(x => Interlocked.Read(ref x.Value.TotalBytes))
            .ToList();
