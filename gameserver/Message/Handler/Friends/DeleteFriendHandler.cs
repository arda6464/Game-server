public static class DeleteFriendHandler
{
    public static void Handle(Session session,byte[] message)
    {
        ...
        if (account != null && targetaccount != null)
        {
            ...
            {
                Logger.genellog($"{account.Username}({account.AccountId}) {targetaccount.Username}({targetaccount.AccountId}) ile zaten arkadaş değil!"); // todo  session toast(silinemedi,arkadaş değilsiniz)
            }
        }
        else
